...
export function ReportsPanel({ orders, production, expenses }: ReportsPanelProps) {
  ...
  const productionVsSalesData = useMemo(() => {
    const dataByDate = new Map<string, { produccion: number, ventas: number }>();
    
    production.forEach(p => {
      const date = new Date(p.date).toISOString().split('T')[0];
      if (!dataByDate.has(date)) dataByDate.set(date, { produccion: 0, ventas: 0 });
