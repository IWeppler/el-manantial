...
const FinalPrice = ({ settings, schedules, mapleOptions }: { settings: SettingsWithTiers; schedules: Schedule[]; mapleOptions: MapleOption[] }) => {
  ...
  const { subtotal, deliveryFee, totalPrice } = useMemo(() => {
    ...
    const selectedOption = mapleOptions.find(opt => opt.value === values.mapleQuantity);
    if (!selectedOption) {
      return { subtotal: 0, deliveryFee: 0, totalPrice: 0 };
    }
    ...
    if (selectedSchedule?.type === "DELIVERY" && (!settings.freeDeliveryThreshold || currentSubtotal < settings.freeDeliveryThreshold)) {
      currentDeliveryFee = settings.deliveryFee;
    }
    
    const currentTotalPrice = currentSubtotal + currentDeliveryFee;
    return { subtotal: currentSubtotal, deliveryFee: currentDeliveryFee, totalPrice: currentTotalPrice };

  }, [values.mapleQuantity, values.scheduleId, settings, schedules, mapleOptions]);
...
const FormContent = ({ isLoggedIn, settings, schedules, mapleOptions }: FormContentProps) => {
    const { values, setFieldValue, isValid, dirty } = useFormikContext<FormValues>();

  const availableDays = useMemo(() => {
    if (!values.deliveryType) return [];
    const days = schedules.filter(s => s.type === values.deliveryType && s.isActive).map(s => s.dayOfWeek);
  ...
  const availableTimeRanges = useMemo(() => {
    ...
    return schedules
      .filter(s => s.type === values.deliveryType && s.dayOfWeek === values.dayOfWeek && s.isActive)
  ...
  return (
    <Form className="space-y-6">
      {!isLoggedIn && (
        <>
          <CustomInput label="Nombre Completo" name="name" type="text" placeholder="Juan Pérez" />
          <CustomInput label="Número de WhatsApp" name="phone" type="tel" placeholder="3491..." />
        </>
      )}

      <CustomSelect label="1. ¿Cuanta cantidad querés?" name="mapleQuantity" placeholder="Selecciona una opción..." options={mapleOptions} />
      
      <CustomSelect label="2. ¿Retirás o te lo enviamos?" name="deliveryType" placeholder="Selecciona el tipo de entrega..." options={[{value: ScheduleType.PICKUP, label: "Retiro en el local"}, {value: ScheduleType.DELIVERY, label: "Envío a domicilio"}]} />

      {values.deliveryType && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <CustomSelect label="3. ¿Qué día?" name="dayOfWeek" placeholder="Selecciona un día..." options={availableDays} />
          {values.dayOfWeek && (
            <CustomSelect label="4. ¿En qué franja horaria?" name="scheduleId" placeholder="Selecciona una hora..." options={availableTimeRanges} />
          )}
        </div>
      )}

      {values.deliveryType === 'DELIVERY' && <CustomInput label="Dirección de Envío" name="address" type="text" placeholder="Calle Falsa 123" />}
      
      <CustomSelect label="5. Método de Pago" name="paymentMethod" placeholder="Selecciona una opción..." options={[{ value: PaymentMethod.CASH, label: "Efectivo" }, { value: PaymentMethod.TRANSFER, label: "Transferencia" }]} />
      
      <FinalPrice settings={settings} schedules={schedules} mapleOptions={mapleOptions} />
      
      <Button type="submit" disabled={!dirty || !isValid} className="w-full mt-4">
        Hacer mi pedido
      </Button>
    </Form>
  );
...
const OrderForm = ({ isLoggedIn, userName, settings, schedules }: OrderFormProps) => {
    ...
    const mapleOptions = useMemo(() => {
        ...
        for (let i = 1; i <= 10; i++) {
            ...
            options.push({ value: i.toString(), label: `${i} Maple${i > 1 ? 's' : ''} (${i * 30}) - ${formatPrice(totalPrice)}`, price: totalPrice });
        }
        return options;
    }, [settings]);
    ...
    const validationSchema = Yup.object({
        ...
        deliveryType: Yup.string().oneOf(Object.values(ScheduleType)).required("Selecciona el tipo de entrega"),
        dayOfWeek: Yup.string().required("Selecciona el día"),
        scheduleId: Yup.string().required("Selecciona la hora"),
        paymentMethod: Yup.string().oneOf(Object.values(PaymentMethod)).required("Selecciona un método de pago"),
        address: Yup.string().when('deliveryType', { is: 'DELIVERY', then: (schema) => schema.required("La dirección es obligatoria para envíos") }),
        ...(!isLoggedIn && {
            name: Yup.string().min(3, "El nombre es muy corto").required("El nombre es obligatorio"),
            phone: Yup.string().matches(/^[0-9]{10,}$/, "El teléfono debe tener al menos 10 dígitos").required("El teléfono es obligatorio"),
        })
    });
   ...
   const handleSubmit = async (values: FormValues, { setSubmitting, resetForm }: FormikHelpers<FormValues>) => {
        ...
        try {
            ...
            if (isAxiosError(err) && err.response) { setError(err.response.data.message || "No se pudo crear la orden."); } 
    ...
    const initialValues: FormValues = {
        name: "", phone: "", address: "", mapleQuantity: "", 
        deliveryType: "", dayOfWeek: "", scheduleId: "", paymentMethod: "" 
    };
        ...
        <>
            <div className="w-full mx-auto">
                ...
                <Formik initialValues={initialValues} validationSchema={validationSchema} onSubmit={handleSubmit}>
                    <FormContent isLoggedIn={isLoggedIn} settings={settings} schedules={schedules} mapleOptions={mapleOptions} />
                </Formik>
